//! Amortization schedule command — Pro liability-payoff report (M4.2).
//!
//! Thin wrapper over `mizan_core::portfolio::amortization::build_schedule`.
//! Gated on `advanced_reports` (Pro+) so the desktop's UpgradeGate raises
//! cleanly for Free/Basic users.
//!
