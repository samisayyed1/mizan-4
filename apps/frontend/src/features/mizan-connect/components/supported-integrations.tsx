/**
 * Mizan Connect — supported integrations panel.
 *
 * Feroz on the May-17 call about empty asset classes:
 *   "if there is an API for it, it should prompt saying that would
 *    you want to connect to this uh you know whatever IBKR or
 *    whatever you know then you let them connect directly."
 *
 * The chooser menu (PR #82) routes "Connect a broker" → `/connect`.
 * Before this PR, the user landed on a connect page that *named no
 * actual brokers* — just abstract "Brokerage Sync" feature pills. A
 * user who has Interactive Brokers had to *guess* whether Mizan
 * supports it. That kills the conversion right at the moment of
 * intent.
 *
 * This component fills the gap with a curated grid of the broker
 * names + logos that SnapTrade exposes. Static data — these are the
 * "highlight" set; SnapTrade's portal itself still lists every
 * provider when the user clicks Connect.
 *
 * Reference: .claude/product-notes/feroz-meeting-2026-05-17.md #7
 * (API / Broker Connection Prompt) and the transcript quote above.
 */
  ...
  return (
    <section aria-labelledby="supported-integrations-heading" className="space-y-3">
      <div className="flex items-center gap-3">
        ...
        >
          Supports the brokers you already use
        </h2>
      ...
      <p className="text-muted-foreground/80 text-center text-xs">
        …and many more. The SnapTrade portal lists every provider when you click
        <span className="font-medium"> Connect a broker</span>.
      </p>
    </section>
  );
}
